...
export default function Result() {
  ...
  // Если открыли напрямую — попробуем загрузить последний результат из БД.
  useEffect(() => {
    if (report) return
    let alive = true
    ;(async () => {
      ...
      try {
        ...
        const { data } = await supabase
          .from('test_results')
          .select('strengths, weaknesses, recommended_professions, full_report')
          .eq('user_id', user.id)
          .order('created_at', { ascending: false })
          .limit(1)
          .maybeSingle()
        if (alive && data) setReport(data)
      } finally {
  ...
  if (loading) {
    return wrap(
      <div className="res-page"><div className="res-empty">Загружаем результат…</div></div>
    )
  }
...
/* Профессии могут прийти как строки или объекты {name, match}. */
