...
export default function Block7SelfEfficacy() {
  ...
  async function submit() {
    ...
    const scores = scoreSelfEfficacy({ GEN, PROF, DEC, CASES, ans, caseAns })
    const { flags, ...rest } = scores
    await CP.saveBlockResult(7, { answers: { likert: ans, cases: caseAns }, scores: { ...rest, flags }, durationSec: dur })
    await CP.updateProfile({ se_general: rest.se_general, se_confidence: rest.se_confidence, se_awareness: rest.se_awareness, decisiveness: rest.decisiveness, decision_style: rest.dominant_style, career_execution: rest.career_execution })
