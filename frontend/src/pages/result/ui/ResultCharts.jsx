...
const TOOLTIP = {
  ...
  border: '1px solid rgba(37,32,92,0.12)',
  borderRadius: 8,
  color: '#25205c',
  fontSize: 12,
  padding: '8px 12px',
  boxShadow: '0 8px 24px rgba(37,32,92,0.14)',
}
    ...
    <div className="chart-block">
      ...
      <ResponsiveContainer width="100%" height={240}>
        <RadarChart data={data} margin={{ top: 10, right: 34, bottom: 10, left: 34 }}>
          <PolarGrid stroke="rgba(37,32,92,0.1)" />
          <PolarAngleAxis dataKey="subject" tick={{ fill: '#626185', fontSize: 10, fontWeight: 600 }} />
    ...
    <div className="chart-block">
      ...
      <ResponsiveContainer width="100%" height={220}>
        <BarChart data={data} margin={{ top: 5, right: 10, left: -20, bottom: 40 }} barCategoryGap="28%">
          <XAxis dataKey="name" tick={{ fill: '#626185', fontSize: 10, fontWeight: 600 }}
            axisLine={false} tickLine={false} interval={0} angle={-20} textAnchor="end" />
          <YAxis domain={[0, 100]} tick={{ fill: '#8583a3', fontSize: 9 }} axisLine={false} tickLine={false} />
          <Tooltip contentStyle={TOOLTIP} formatter={v => [`${v}%`, 'Балл']} cursor={{ fill: 'rgba(37,32,92,0.03)' }} />
    ...
    <div className="chart-block">
      ...
      <ResponsiveContainer width="100%" height={data.length * 46 + 20}>
        <BarChart data={data} layout="vertical" margin={{ top: 4, right: 52, left: 4, bottom: 4 }} barCategoryGap="22%">
          ...
          <XAxis type="number" domain={[0, 100]} tick={{ fill: '#8583a3', fontSize: 9 }} axisLine={false} tickLine={false} tickFormatter={v => `${v}%`} />
          <YAxis type="category" dataKey="name" width={155} tick={{ fill: '#25205c', fontSize: 11, fontWeight: 600 }} axisLine={false} tickLine={false} />
          <Tooltip contentStyle={TOOLTIP} formatter={v => [`${v}%`, 'Совпадение']} cursor={{ fill: 'rgba(37,32,92,0.03)' }} />
          <Bar dataKey="match" radius={[0, 5, 5, 0]} fill={`url(#${GRAD_ID})`}
            background={{ fill: 'rgba(37,32,92,0.05)', radius: [0, 5, 5, 0] }}
