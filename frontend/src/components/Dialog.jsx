...
const btnAccent = { ...btnBase, background: 'var(--accent,#00e5c8)', color: '#08080f' }
const btnDanger = { ...btnBase, background: 'var(--danger,#ff4d6d)', color: '#fff' }
const btnGhost = { ...btnBase, background: 'transparent', color: 'var(--sub,#9898b8)', border: '1px solid var(--line2,rgba(255,255,255,.12))' }
...
export function DialogHost() {
  ...
  return (
    <div onClick={onOverlay} style={{ position: 'fixed', inset: 0, zIndex: 1000, background: 'rgba(0,0,0,.65)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20, animation: 'cpDlgFade .15s ease' }}>
      <div role="dialog" aria-modal="true" style={{ width: '100%', maxWidth: 400, background: 'var(--card,#111120)', border: '1px solid var(--line2,rgba(255,255,255,.12))', borderRadius: 16, boxShadow: '0 24px 70px rgba(0,0,0,.6)', padding: 24, fontFamily: "'Manrope',sans-serif", animation: 'cpDlgIn .18s ease' }}>
        {d.title && <div style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: 24, letterSpacing: 1.5, color: 'var(--text,#ecedf8)', marginBottom: 10 }}>{d.title}</div>}
        <div style={{ fontSize: 14, lineHeight: 1.6, color: 'var(--sub,#9898b8)', whiteSpace: 'pre-line' }}>{d.message}</div>
