...
export async function generateRoadmap(profile, professions) {
  ...
  const explicitUrl = import.meta.env.VITE_ROADMAP_URL
  const baseUrl = import.meta.env.VITE_SUPABASE_URL
  const useRemote = isSupabaseConfigured || (explicitUrl && baseUrl)

  if (useRemote) {
    const fnUrl = explicitUrl || `${baseUrl}/functions/v1/career-roadmap`
    try {
      ...
      const res = await fetch(fnUrl, {
        ...
        body: JSON.stringify({ profile, professions, age }),
      })
      if (!res.ok) throw new Error(`career-roadmap ${res.status}: ${await res.text().catch(() => '')}`)
      const data = await res.json()
      if (data.error) throw new Error(data.error)
      if (!data.steps?.length) throw new Error('пустой маршрут')
      return { ...data, _source: 'yandexgpt' }
    } catch (e) {
      return { ...localRoadmap(profile, professions), _source: 'local-fallback', _error: String(e) }
    }
  }
  return { ...localRoadmap(profile, professions), _source: 'local' }
}
